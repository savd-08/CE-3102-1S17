...
namespace anpi{
	...
	//Solución de Q·b = b_x (multiplicación matriz-vector)
	template<typename T>
	void qmult(const Matrix<T>& Q, const std::vector<T>& b, std::vector<T>& b_x) {
		int i,j;
		T sum;
		int n = Q.rows();
		for (i = 0; i < n; i++) {
	...
	template<typename T>
	T testQR(const Matrix<T>& A, const Matrix<T>& Q, const Matrix<T>& R){
		Matrix<T> A_x = Q*R;
