...
/********************************************************************************************************************************/
	...
	*/
	...
	public:
		//Constructor
		laguerre(polynomial<std::complex<T>> ppoly, bool ptype, unsigned int piters = 100, T pprecision = 0.00001) {
		...
		polynomial<std::complex<T>> operator() (T lim_inf) {
			...
			for (int i = 0; i < _iters; i++) {
				...
				if(i >= 1) g = std::complex<T>(-1.0,0.0) * dpol(tmp_root, _poly) / poly_evaluator(tmp_root, _poly); //Verificacion de signo para
				...
				if ( std::abs((new_root - tmp_root) / new_root) <= _precision ) {
					...
					//Se verifica si el resultado calculado no es un numero (advertencia: NaN)
					if (_type) {
						if (new_root.imag() != T(0)) {
							std::cout << "Advertencia: la raíz es obtenida no es un número real." << std::endl;
						}
					}
